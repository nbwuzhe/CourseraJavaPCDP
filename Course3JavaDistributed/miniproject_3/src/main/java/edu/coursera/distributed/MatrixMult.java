...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        // Broadcast input matrix a and b to all nodes
        ...
        // Assembling the results from all nodes.
        if (myRank == 0) {// if this is the master node, we collect the results from all other nodes
            MPI.MPI_Request[] requests = new MPI.MPI_Request[size - 1];
            for (int r = 1; r < size; r++) {
                final int rankStartRow = r * rowChunk;
                int rankEndRow = (r + 1) * rowChunk;
                if (rankEndRow > nRows) {
                    rankEndRow = nRows;
                }
                ...
                requests[r - 1] = mpi.MPI_Irecv(c.getValues(), rowOffset, nElements, r, r, mpi.MPI_COMM_WORLD);
            }

            mpi.MPI_Waitall(requests);
        } else { // When it's not master node, just send the data to the master node.
