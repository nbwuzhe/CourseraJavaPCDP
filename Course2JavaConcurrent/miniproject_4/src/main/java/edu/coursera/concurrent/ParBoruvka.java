...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while ((n = nodesLoaded.poll()) != null) {
            // Try to lock the current node to start working on it.
            if (!n.lock.tryLock()) {
                continue; // Don't re-add to nodesLoaded list since n is being occupied by another thread
            }

            // If the component/node has been merged to another one.
            if (n.isDead) {
                n.lock.unlock();
                continue;
            }
            ...
            // If the graph has been contracted to a single node
            if (e == null) {
            ...
            // If the other node on the edge is being processed on another thread
            if (!other.lock.tryLock()) {
                n.lock.unlock();
                nodesLoaded.add(n); // We need to put the current node back to the list since it hasn't been processed
                continue;
            }

            // If the other node on the edge has been merged
            if (other.isDead) {
                n.lock.unlock();
                other.lock.unlock();
                nodesLoaded.add(n);
            ...
            // At this point, locks for both n and other nodes are acquired
            other.isDead = true;
            n.merge(other, e.weight());

            // Release locks for both nodes
            n.lock.unlock();
            other.lock.unlock();

            // Add the new (merged) node to the nodesLoaded list
            nodesLoaded.add(n);
        }
    ...
    public static final class ParComponent extends Component<ParComponent> {
        /**
         *  A lock to indicate if the current component is being occupied by a thread.
         *  Use fairness policy
         */
